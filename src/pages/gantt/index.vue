...
export default {
  ...
  methods: {
    _getTasksModel() {
      this.$http.post(ganttModelURL).then(res=>{
          console.log(res)
          this.isLoading = true
          let resData = res.ganttData;
          let resLinks = res.ganttMainData;
          let taskData = [];
          let links = [];
          resData.map(item=>{
            taskData.push(
              {
                id: item.PMPLAN_PROJECTSCHEDULE_ID,
                text: item.PMPLAN_PS_JOBCONTENT,
                start_date: new Date(item.PMPLAN_PS_STARTDATE),
                end_date: new Date(item.PMPLAN_PS_ENDDATE),
                progress: item.PMPLAN_PS_PRO_STAGE,
                parent: item.PMPLAN_P_ID,
                person: [item.RESOURCES_DICTNAME],
                // user: item.RESOURCES_DICTNAME
              }
            )
          ...
          resLinks.map(item=>{
            links.push({
              id: item.PMMANAGE_PROJECTMANAGE_ID,
              target: item.PMMANAGE_PM_PROJECTID,
              type: '0',
              source: item.PMMANAGE_PROJECTMANAGE_ID
            })
          })
          this.tasks = {
            data: taskData,
